...
variable "create_frontdoor" {
  description = "Boolean to determine whether to create the Front Door"
  type        = bool
  default     = true
}

variable "frontdoor_name" {
  description = "The custom name for the Front Door profile"
  type        = string
}
...
variable "logs_destinations_ids" {
  description = "List of destination resource IDs for sending logs"
  type        = list(string)
  default     = []
}
...
variable "frontdoor_custom_domains" {
  ...
  validation {
    condition = alltrue([
      for frontdoor_custom_domains in var.frontdoor_custom_domains :
      can(regex("^[a-zA-Z0-9][0-9A-Za-z-]*[a-zA-Z0-9]$", frontdoor_custom_domains.name)) &&
      length(frontdoor_custom_domains.name) >= 2 &&
      length(frontdoor_custom_domains.name) <= 260
    ])
...
variable "frontdoor_security_policies" {
  ...
  validation {
    condition = alltrue([
      for frontdoor_security_policies in var.frontdoor_security_policies :
      frontdoor_security_policies.custom_domain_names != null ||
      frontdoor_security_policies.endpoint_names != null
    ])
